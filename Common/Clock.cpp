...
{
    std::chrono::system_clock::time_point EgLab::Common::Clock::now()
    {
    ...
    void Clock::time(String& hour, String& minute, String& second)
    {
        ...
        std::tm* ltm = std::localtime(&nowTime);
        if (ltm->tm_hour < 10)
            hour = "0" + EgLab::Common::ToString(ltm->tm_hour);
        else
            hour = EgLab::Common::ToString(ltm->tm_hour);

        if (ltm->tm_min < 10)
            minute = "0" + EgLab::Common::ToString(ltm->tm_min);
        else
            minute = EgLab::Common::ToString(ltm->tm_min);

        if (ltm->tm_sec < 10)
            second = "0" + EgLab::Common::ToString(ltm->tm_sec);
        else
            second = EgLab::Common::ToString(ltm->tm_sec);
    }
    ...
    void Clock::delta_time(std::chrono::high_resolution_clock::time_point t1,
                           ...
                           int& minute, int& second)
    {
        ...
        minute = std::chrono::duration_cast<std::chrono::minutes>(duration % std::chrono::hours(1))
                     .count();
        second =
    ...
    double Clock::delta_time(std::chrono::high_resolution_clock::time_point t1,
